...
#import "DTAnimatedImageView.h"
#import "DTImage.h"

@interface DTAnimatedImageView ()
...
@implementation DTAnimatedImageView
...
- (CADisplayLink *)displayLink {
    ...
    if (self.superview) {
        if (_displayLink && _animatedImage) {
...
- (void)setImage:(UIImage *)image {
    ...
    self.accumulator = 0;
    if ([image isKindOfClass:[DTImage class]] && image.images) {
        if ([image.images[0] isKindOfClass:[UIImage class]]) {
            [super setImage:image.images[0]];
            self.currentFrame = nil;
            self.animatedImage = (DTImage *)image;
...
- (void)setAnimatedImage:(DTImage *)animatedImage {
...
- (BOOL)isAnimating {
    return [super isAnimating] || (self.displayLink && !self.displayLink.paused);
}
