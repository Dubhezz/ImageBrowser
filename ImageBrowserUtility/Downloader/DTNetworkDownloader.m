...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveData:(NSData *)data {
    ...
    if (completion)
    {
        ...
        if (progress < 1)
        {
            completion(request.URL.absoluteString, recvdata, progress, nil);
        }
    }
}
...
//    [self.reqTable removeObjectForKey:request.keyForLoader];
    ...
    if (error)
    {
        if (error.code != NSURLErrorCancelled)
        {
            ...
            else
            {
                if (completion)
                {
                    completion(request.URL.absoluteString, nil, 1, error);
                }
            }
    ...
    else
    {
        ...
        if (completion)
        {
            completion(request.URL.absoluteString, data, 1, error);
        }
    }
}
...
+ (NSString *)cacheFilePathForURL:(NSString *)URL
{
    ...
    [[NSFileManager defaultManager] createDirectoryAtPath:path
                              withIntermediateDirectories:true
                                               attributes:nil
                                                    error:nil];
    return [path stringByAppendingPathComponent:[NSString MD5:URL]];
}
